...
class World:
    def __init__(self, carla_world, hud, args):
        ...
        self.trajectory_lifetime = 0.1  # Lifetime of trajectory visualization in seconds
    ...
    def visualize_trajectory(self):
        """Visualize the predicted trajectory in the world"""
        if not self.predicted_trajectory:
            return
            
        # Set color for trajectory visualization
        color = carla.Color(r=0, g=255, b=0)  # Green for predicted trajectory
        
        # Draw each point of the trajectory
        for i in range(len(self.predicted_trajectory) - 1):
            start = carla.Location(x=self.predicted_trajectory[i][0], 
                                   y=self.predicted_trajectory[i][1], 
                                   z=self.player.get_location().z + 0.5)
            end = carla.Location(x=self.predicted_trajectory[i+1][0], 
                                y=self.predicted_trajectory[i+1][1], 
                                z=self.player.get_location().z + 0.5)
            
            # Draw line between consecutive points
            self.debug_helper.draw_line(start, end, thickness=0.2, color=color, life_time=self.trajectory_lifetime)
...
class VehicleControl:
    ...
    def _handle_manual_control(self, world):
        ...
        if ready_to_go:
            ...
            world.predicted_trajectory = world.controller.get_trajectory()
...
class CameraManager:
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        # Camera setup - Add more camera angles for different views
        ...
        self._camera_transforms = [
            ...
            # Add an overhead view for better trajectory visualization
            (carla.Transform(carla.Location(x=0, z=50.0), carla.Rotation(pitch=90.0)), carla.AttachmentType.SpringArm)
        ]
        
        self.transform_index = 5  # Default to overhead view for trajectory visualization
        self.sensors = [
...
def game_loop(args):
    ...
    try:
        ...
        pygame.display.set_caption("CARLA MPC Controller with Trajectory Visualization")
...
def main():
    ...
    # Spawn settings
    argparser.add_argument('--spawn_x', metavar='X', default='-360', help='x position to spawn the agent (default: -360)')
    argparser.add_argument('--spawn_y', metavar='Y', default='35', help='y position to spawn the agent (default: 35)')
    ...
    argparser.add_argument('--planning_horizon', metavar='HORIZON', type=int, default=10,
